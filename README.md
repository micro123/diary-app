...
- **标签自动化规则**：按 Tracker 实例配置「添加标签 → 设置字段」规则，可选择仅补空值或强制修改已有值
- **统计页**：工时分布、标签明细
...
- **脚本系统**：C#、Lua、Python 三种语言的脚本，支持应用脚本（手动执行）、编辑器脚本（日历右键按日/周/月/季/年/事项执行）、自动化脚本（`daily HH:mm` 定时 + 启动补跑）与查询脚本（只读统计汇总）；可生成带本地标签/字段/模板的 AI 上下文包，并通过只读快照型 stdio MCP 辅助 AI 编写和无执行校验脚本。API 参考见 [`Docs/ScriptApi`](Docs/ScriptApi/CSharp.md)
