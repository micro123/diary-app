...
internal class PgMigrator: IDisposable, IAsyncDisposable
{
    ...
    {
        using var command = Command("SELECT version_code FROM _version_ ORDER BY version_code DESC LIMIT 1;");
    ...
    {
        using var command = Command("SELECT * FROM redmine_activities;");
    ...
    {
        using var command = Command("SELECT * FROM work_items;");
        using var reader = command.ExecuteReader();
        var dummyId = 1;
        var cnt = 1;
        while (reader.Read())
    ...
    {
        using var command = Command("SELECT * FROM tags;");
        using var reader = command.ExecuteReader();
        var cnt = 1;
        while (reader.Read())
        {
    ...
    {
        using var command = Command("SELECT * FROM work_item_tags;");
        using var reader = command.ExecuteReader();
        var cnt = 1;
        while (reader.Read())
        {
