...
当前已经实现查询模型、SQLite/PostgreSQL provider、跨数据库契约测试、独立查询页面以及统计详情复用。
保存查询、结果跳转和脚本只读查询仍在待办中。
...
`Limit > 0` 时应用分页，负数 `Offset` 归零。稳定排序保证同一查询条件下分页不会因为未定义行顺序而漂移。
...
- 增加查询异常状态，失败时保留上一次成功结果。
- 支持从查询结果定位到对应日期和事项。
- 增加批量标签加载，避免结果页逐事项查询标签。
- 设计保存查询配置，支持新增、重命名、编辑和删除。

后续：

- 向受限脚本 API 暴露只读 `QueryWorkItems`。
- 为脚本查询增加权限、异常和敏感信息测试。
- 评估总数查询和真正的分页结果模型。
