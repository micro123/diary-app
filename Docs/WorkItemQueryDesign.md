...
- 批量加载结果标签，避免逐事项查询。
- 结果导出为 CSV 或 Markdown，并可复制查询汇总文本。
...
`ScriptWorkItemQuery` 的 `Range` 支持日期范围快捷值（`today`、`yesterday`、`thisweek`、
`thismonth`，不区分大小写），解析后展开为对应的 `StartDate`/`EndDate`。
