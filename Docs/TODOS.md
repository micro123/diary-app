...
- [~] 第三阶段：已增加首次成功启动引导、“以后不再显示”选项、设置页重新打开入口和“显示开发者功能”开关，普通用户默认隐藏脚本管理页；Tracker 动态导航继续按已启用实例构建。调查功能已区分调查者和受访者角色：调查者显示调查页并监听固定端口 9721，受访者填写调查者 IP 后不显示调查页。脚本试运行、副作用预览和执行历史体验仍待补充。
