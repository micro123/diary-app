...
- [ ] 定义 `WorkItemQuery`，至少包含开始日期、结束日期、标签 ID 集合、标签匹配方式、关键字、优先级、分页参数。
- [ ] 定义 `WorkItemTagFilter` 或等价枚举，区分 `Ignore`、`Any`、`All`、`None`、`Exact`。
- [ ] 为 `DbInterfaceBase` 增加统一的 `QueryWorkItems(WorkItemQuery query)` 抽象接口。
- [ ] 在 SQLite provider 实现日期、标签、关键字、优先级和分页查询。
- [ ] 在 PostgreSQL provider 实现与 SQLite 等价的查询语义和参数绑定。
- [ ] 保留现有 `GetWorkItemsByTagAndDate` 的兼容行为，或将统计调用迁移到新接口后再删除旧接口。
- [ ] 查询结果使用日期和工作项 ID 的稳定排序，避免多标签 JOIN 造成重复工作项。
- [ ] 为空条件定义明确语义：无标签条件不能与忽略标签条件混淆。

### 7.2 数据库契约测试

- [ ] 查询单个标签时只返回具有该标签的工作项。
- [ ] `Any` 匹配多个标签时返回具有任意一个标签的工作项。
- [ ] `All` 匹配多个标签时只返回同时具有全部标签的工作项。
- [ ] `None` 匹配时只返回没有任何标签的工作项。
- [ ] `Exact` 匹配时只返回标签集合完全一致的工作项。
- [ ] 日期范围包含开始日期和结束日期。
- [ ] 日期范围外的工作项不会返回。
- [ ] 一个工作项具有多个匹配标签时结果中只出现一次。
- [ ] 关键字、优先级和标签条件组合时结果正确。
- [ ] 查询无结果时返回空集合而不是 `null`。
- [ ] SQLite 和 PostgreSQL 契约测试返回相同结果。
- [ ] 查询参数全部使用 provider 参数绑定，禁止字符串拼接用户输入。

### 7.3 查询 UI 和保存查询

- [ ] 增加自定义查询页面或对话框，支持开始日期和结束日期。
- [ ] 支持标签多选以及任意/全部/无标签匹配方式。
- [ ] 支持按标题或备注关键字查询。
- [ ] 支持按优先级筛选。
- [ ] 结果列表显示日期、标题、耗时、优先级和标签。
...
- [ ] 统计页面迁移到统一查询接口，避免继续维护独立标签查询 SQL。
