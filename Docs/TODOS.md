...
- [~] 第二阶段：已增加复制昨天、15/30 分钟及 1/2/4 小时快捷工时输入，并让新建事项优先展示当天最近使用标签；查询已增加今天/昨天/本周/本月快捷条件、耗时合计和 CSV/Markdown 导出，查询页顶部已按保存查询、筛选条件和结果操作分层重排，筛选条件支持折叠。最近项目持久化、复制最近一条/整天复制、按项目/日期分组汇总和同步状态快捷筛选仍待补充。
