...
- [~] SQLite 已支持手动创建、校验和下次启动还原完整数据库，并在启动复检失败时恢复还原前安全副本；PostgreSQL 已接入 Client `bin` 目录配置、Windows/Linux 工具探测、custom-format dump/restore、工具版本检查、最小权限预检和非覆盖式还原；目标切换与更完整的跨版本兼容性门禁仍待补齐。应用包增量更新仍待实现。Windows/Linux 终止性托管异常已通过独立 DiagnosticsClient 进程生成 Triage Dump，并显示最小崩溃提示窗口
...
- [~] PostgreSQL 已实现 `pg_dump`/`pg_restore`、工具/服务端主版本检查、当前操作所需的最小权限预检、自动创建目标数据库，以及无 `CREATEDB` 时恢复到已有空数据库；后续补充独立目标库切换、目标兼容性复检和更完整的进程取消/超时测试；不覆盖当前数据库。
