...
- [ ] 完成应用更新第二阶段：客户端逐 Blob 增量下载、缓存和并发控制；服务端事务恢复、下载租约和完整管理鉴权；使用上一发布版本完成 Windows、Linux 和 Windows Python flavor 的真实更新及回滚门禁。
...
- [ ] 继续审计 UI 和服务层剩余的 fire-and-forget 入口，确保异常进入日志或结构化诊断，关闭时可等待或取消后台任务。
...
- [ ] 为脚本交互式 XLSX/CSV/DOCX/Mustache 导出补充真实 UI 端到端测试。
