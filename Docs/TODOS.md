...
- [~] 第三阶段：已增加首次成功启动引导、“以后不再显示”选项、设置页重新打开入口和“显示开发者功能”开关，普通用户默认隐藏脚本管理页；Tracker 动态导航继续按已启用实例构建。调查功能已区分调查者和受访者角色：调查者显示调查页、监听固定端口 9721，并通过 localhost 作为受访者接收自己的调查结果；普通受访者填写调查者 IP 后不显示调查页。脚本执行历史已实现（内存保留最近 30 条，支持状态/来源筛选、清空和复制日志）；剩余待补充仅为试运行（DryRun）、副作用预览，以及执行历史的持久化和关联 worker/request/execution ID。
