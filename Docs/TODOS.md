...
- [ ] 将无外部依赖的 Windows/Linux CDP UI 套件纳入定期门禁，并为系统托盘、文件/目录选择器、剪贴板和真实备份还原补充平台原生自动化或明确的发布前检查单；Windows 已完成统一 UI 后的 9/9 常规套件（8 个结构化套件共 74 步，加 smoke）和 5 步 Redmine 只读视觉回归，core 14/14 已覆盖固定一周、滚轮逐周浏览、日期日/周菜单、月份标题月/季度/年度菜单、完整月历高度、编辑器字段对齐及跨月浏览后回到今天，手册截图已使用真实窗口表面修复 overlay 重复缩放并统一为逻辑 1×/96 DPI，物理原图仍保留；Linux 已补生命周期工具并通过 X11 下 core 14/14、smoke 与 extended 11/11（含 AI 上下文、MCP 快照和配置复制），仍需完成 Xvfb headless 全量编排和 CI 稳定性门禁。
