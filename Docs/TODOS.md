...
- [x] 实现脚本目录扫描、扩展名匹配、元数据读取和启用/禁用状态加载。
...
- [x] 实现源码旁 JSON 元数据和 `manifest.json` 脚本包（支持入口、启用状态、API 版本、ID、名称、描述和能力校验）。
- [x] 实现基于引擎名称、引擎版本、源码哈希、脚本契约版本和权限策略的缓存键。
- [x] 源码、引擎、契约或权限策略变化时使缓存失效。
- [x] 缓存使用临时文件写入和原子替换，损坏缓存自动删除并重新构建。
- [x] 提供脚本列表、启用/禁用、重新加载、编译检查和错误详情 UI。
...
- [x] 脚本目录发现、元数据错误、重复 ID 和启用/禁用状态测试。
...
- [ ] 将脚本页改为“脚本列表 + 脚本概览/诊断/执行历史”的工作台布局。
- [ ] 列表显示用户可读的脚本类型、引擎、启用状态、构建状态和能力标签，隐藏内部枚举值。
- [ ] 增加按名称/ID搜索，以及应用/编辑器、启用/禁用、加载/构建失败筛选。
- [ ] 增加脚本为空、正在加载、加载失败和保留上次成功结果等状态。
- [ ] 加载新结果成功后再替换旧列表，避免重新加载期间页面闪空。
- [ ] 将启动加载诊断同步到脚本页，不只写入日志。
- [ ] 增加“打开脚本目录”入口。
- [ ] 增加脚本执行中的状态、耗时、取消按钮和防重复执行。
- [ ] 统一显示成功、失败、取消、超时和 worker 故障结果，并提供查看诊断入口。
- [ ] 诊断支持错误/警告分类、复制详情、脚本路径和行列信息展示。
- [ ] 执行历史显示脚本名称、来源、开始时间、耗时和结果，避免只显示内部状态名称。

范围执行：

- [ ] 脚本管理页增加目标选择：当前工作项、指定日期、固定周期和自定义日期范围。
- [ ] 自定义范围提供开始日期和结束日期，并显示最终执行摘要。
- [ ] 根据脚本 Scope 和目标能力隐藏不适用的目标选项。
- [ ] 应用脚本和编辑器脚本在管理页使用一致的执行、取消和结果反馈。
- [ ] 对未保存工作项显示“请先保存当前事项”，不假装脚本已处理编辑器草稿。

日历右键菜单：

- [x] 已支持按天、月、年构造编辑器脚本上下文。
- [ ] 将脚本入口统一放入“脚本”子菜单，避免脚本数量增加后占满菜单。
- [ ] 菜单只显示已启用且构建成功的编辑器脚本。
- [ ] 菜单文案明确显示“对当天/当前月份/当前年份运行”。
- [ ] 脚本执行中禁用重复触发，并使用统一的执行结果反馈。

工作项列表右键菜单：

- [ ] 在日志编辑页左下当前日期工作项列表项上增加上下文菜单。
- [ ] 增加“对当前事项运行脚本”子菜单，只显示编辑器脚本。
- [ ] 传入当前工作项日期、`ScriptBusinessTargetKind.WorkItem` 和工作项 ID。
- [ ] 新建未保存工作项的脚本操作禁用，并提示先保存。
- [ ] 当前工作项已锁定时，仍允许只读脚本运行，但不开放任何写入操作。
- [ ] 增加菜单脚本过滤、上下文参数和执行失败测试。

脚本创建和编辑：

- [ ] 脚本管理页增加“新建脚本”向导。
- [ ] 向导支持名称、稳定 ID、应用/编辑器类型、语言、描述和能力选择。
- [ ] 第一阶段支持生成 C# 源码模板和相邻 metadata 文件。
- [ ] 校验 ID 唯一性、文件名安全性和目标目录，禁止静默覆盖已有脚本。
- [ ] 源文件和 metadata 使用原子写入，创建失败不留下半个脚本。
- [ ] 创建完成后提供打开脚本文件、打开目录、编译检查和运行入口。
- [ ] 第一阶段使用外部编辑器编辑源码，不在创建流程中强制集成代码编辑器。
