...
- [~] 已定义稳定 ID、名称、API 版本、范围和能力描述符；源码、引擎和启用状态由后续目录元数据补充。
- [~] 已定义应用和编辑器范围；工作项、周期和自定义日期范围待扩展。
- [~] 已定义应用和编辑器目标；项目、Tracker Issue 和 Tracker 实例待扩展。
- [~] 已定义最小 `EditorScriptContext` 日期范围；粒度和业务目标待扩展。
...
- [~] 每次执行已创建独立执行 ID、取消令牌和超时策略；日志上下文待接入。
