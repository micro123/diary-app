...
- [~] 第一阶段：已在工作记录列表和编辑区显示本地保存与上传摘要状态；Jira/Redmine 已持久化逐 Tracker 最近一次状态、错误和尝试时间；批量同步已增加预览、逐条选择、确认、逐条结果和仅重试已确认失败项；远程 ID/同步时间展示、Tracker 实例筛选和结果不确定项查询仍待补充。
...
- [~] 第二阶段：已增加复制昨天、15/30 分钟及 1/2/4 小时快捷工时输入，并让新建事项优先展示当天最近使用标签；查询已增加今天/昨天/本周/本月快捷条件、耗时合计和 CSV/Markdown 导出，查询页顶部已按保存查询、筛选条件和结果操作分层重排，筛选条件支持折叠；统计页已重排日期控制、自定义工时、工时分布和标签明细区域。最近项目持久化、复制最近一条/整天复制、按项目/日期分组汇总和同步状态快捷筛选仍待补充。
- [~] 第三阶段：已增加首次成功启动引导、“以后不再显示”选项、设置页重新打开入口和“显示开发者功能”开关，普通用户默认隐藏脚本管理页；Tracker 动态导航继续按已启用实例构建。调查功能已区分调查者和受访者角色：调查者显示调查页、监听固定端口 9721，并通过 localhost 作为受访者接收自己的调查结果；普通受访者填写调查者 IP 后不显示调查页。脚本试运行、副作用预览和执行历史体验仍待补充。
- [~] 调查协议：已保留 DiaryToolpp 兼容的 v1/9721 日期查询，并增加新版 v2/9722 自定义统计查询（关键词、标签、标签模式和优先级）；扩展查询只发送到新版节点；已增加 v2 能力发现和调查页能力摘要。更多分组维度和扩展结果明细仍待补充。
