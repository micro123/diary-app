...
- [ ] 将无外部依赖的 Windows/Linux CDP UI 套件纳入定期门禁，并为系统托盘、文件/目录选择器、剪贴板和真实备份还原补充平台原生自动化或明确的发布前检查单；Windows 本机全量 9 套件/72 个结构化步骤已经验收，当前套件定义为 74 步；Linux 已补生命周期工具并通过 X11 下 core 14/14、smoke 与 extended 11/11（含 AI 上下文、MCP 快照和配置复制），仍需完成 Xvfb headless 全量编排和 CI 稳定性门禁。
...
- [ ] 分析远程 PostgreSQL 下启用真实 Redmine 后日期切换 P50/平均值约翻倍、P95 达 301.78 ms 的问题；分别采样 Tracker 绑定查询、每事项编辑扩展创建、选项同步和视觉树构造，目标是在保留真实插件路径的前提下让 P95 稳定低于 300 ms，并在 Windows 与 Linux 各复测至少三轮。
