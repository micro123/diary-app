...
- [ ] 定义标签实际新增事件，区分用户添加、模板添加和批量添加来源。
- [ ] 将手动添加标签和应用模板添加标签统一接入同一个标签添加服务。
- [ ] 加载已有标签、删除标签和重新加载工作项不得触发自动规则。
- [ ] 定义 `TagAutomationContext`、`TagAutomationResult` 和 `ITagAutomationCoordinator`。
- [ ] 将规则存储在 Tracker 实例配置中，支持一个标签关联多个实例。
- [ ] 支持同一 Tracker 实例配置多条规则、启用/禁用和优先级。
- [ ] 默认使用 `OnlyIfUnset`，用户后续手动修改字段不被规则覆盖。
- [ ] 删除标签不反向清除或恢复 Tracker 字段。
- [ ] Redmine 实现实例级标签规则，至少支持标签到 Activity/Issue 默认值映射。
- [ ] 规则字段和动作由 Tracker 插件解释，核心不得出现 Redmine 专用字段。
- [ ] 支持规则按标签添加顺序逐条应用，并基于前一条规则的最新编辑器状态。
- [ ] 设计同字段多规则冲突、无效目标和 Tracker 实例未启用时的处理。
- [ ] 在 Tracker 实例设置页提供规则新增、编辑、删除、启用/禁用和优先级调整。
- [ ] 在核心标签编辑器提供 Tracker 规则扩展入口，按标签查看关联实例规则。
- [ ] 两个规则编辑入口共享编辑 ViewModel 或编辑服务，避免配置互相覆盖。
- [ ] 规则配置支持 schema 迁移、未知字段保留和敏感信息保护。
- [ ] 规则应用只修改当前 Tracker 编辑器草稿，不在标签添加时调用远程 API。
- [ ] 规则修改后的最终 Tracker 字段随现有工作项本地事务保存。
- [ ] 增加手动添加、模板添加、顺序、多实例、多规则和用户覆盖测试。
