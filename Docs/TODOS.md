...
- [x] 普通日志项和模板日志项已具备追加结果、预览、幂等和基础失败恢复边界；宿主共享持久化幂等存储，并保留副作用摘要作为提交结果审计信息。
- [x] 已评估 Tracker 远程写入暂不纳入当前脚本 API；当前只提供 Tracker 实例和能力的只读发现，远程写入另列独立功能，避免扩大脚本副作用边界。
- [x] 普通日志项和模板日志项支持幂等键；`ScriptIdempotencyStore` 按 API 类型隔离键并持久化已提交结果，应用重启后仍能去重。
- [x] 增加 `Preview` 能力，只返回待追加记录和副作用摘要，不写入数据库。
- [x] 执行结果携带 `ScriptEffectSummary`，便于用户判断是否已经追加成功。
- [x] Worker 不自动重试带副作用的请求，避免超时后重复创建。
- [x] 当前业务不需要脚本更正历史记录，暂不提供修正/冲正 API；如未来确有需求，再以独立追加式模型设计，保留原始记录和关联关系。
...
- [x] 已为 C#、Lua、Python 提供“5 分钟入门”和“查询并追加日志项”完整示例，并从各语言 Reference 链接。
- [x] API Reference 已补充入口类型、适用作用域、取消、幂等、预览、领域 API 发现和错误处理说明，并链接三种语言的完整入门示例。
...
- [x] 已覆盖普通日志项和模板日志项的幂等/预览基础行为，补充持久化幂等存储、跨重启读取和 API 类型隔离测试；修正/冲正当前明确不在实施范围。
