...
- [ ] 完成应用更新后续优化：客户端逐 Blob 增量、完整包兜底和状态栏下载进度已接入，继续实现跨事务内容缓存、并发控制、断点续传和可取消/恢复的详细事务视图；服务端补事务恢复、下载租约和完整管理鉴权；Windows standard 的 local 增量、完整包兜底和 MCP 占用回滚已验收，继续完成 Windows Python flavor、Linux、初始化失败及进程中止恢复门禁。
...
- [ ] 将无外部依赖的 Windows/Linux CDP UI 套件纳入定期门禁，并为系统托盘、文件/目录选择器、剪贴板和真实备份还原补充平台原生自动化或明确的发布前检查单；Windows 已完成统一 UI 后的 9/9 常规套件（8 个结构化套件共 74 步，加 smoke）和 5 步 Redmine 只读视觉回归，core 14/14 已覆盖日期操作按钮左右贴边、固定一周、圆形今天/选中/悬停高亮、滚轮逐周浏览、非选中日期右键选中并打开日/周菜单、今天与当前选中状态分离、月份标题月/季度/年度菜单、完整月历模板不裁切及底边完整、重新展开恢复月视图、相邻月份日期精确选择并自动关闭、编辑器字段对齐及跨月浏览后回到今天，手册截图已使用真实窗口表面修复 overlay 重复缩放并统一为逻辑 1×/96 DPI，物理原图仍保留；Linux 已补生命周期工具并通过 X11 下 core 14/14、smoke 与 extended 11/11（含 AI 上下文、MCP 快照和配置复制），仍需完成 Xvfb headless 全量编排和 CI 稳定性门禁。
  ...
  - 2026-08-26 已按数据版本 `1.0.1` 全面复核并细化用户手册，更新当前界面图片且只截取目标控件或对话框关键区域；文档工作已归档到 `CompletedWork.md`，不增加新的功能待办。
  - 同日精简 V2 参数表单字段辅助说明为“参数键 · 类型”，来源状态继续仅供恢复默认值和参数记忆使用；该项已归档到 `CompletedWork.md`，不增加新的待办。
  - 同日清理 3 处依赖小尺寸信息图标触发 Tooltip 的说明，改为直接显示或由现有状态表达；该项已归档到 `CompletedWork.md`，不增加新的待办。
  - 同日修复 RID 发布时 Script Worker 未显式还原导致的 `NETSDK1047`，同步本地工具和 CI 工作流；该项已归档到 `CompletedWork.md`，不增加新的待办。
  - 同日日历右键菜单补充脚本分组分隔符和禁用“暂无”空状态；该项已归档到 `CompletedWork.md`，不增加新的待办。
  - 同日新增主导航冷热切换 CDP 性能场景、单进程采样器和跨进程汇总工具；本机 Linux core 流程已实际串通，该项已归档到 `CompletedWork.md`，不增加常规全量门禁要求。
  - 同日完成主导航每 ViewModel 实例一个 View 的缓存、真实视觉树常驻预热、页面预加载契约、统计页签按需初始化和脚本管理后台准备；缓存资格默认关闭且仅由主导航页面显式加入，数量较大的事项编辑器不会进入缓存。Linux X11 核心与多进程性能复测通过；统计页首次显示仍包含当前图表/明细树创建成本，脚本页真实 HDD 数据可依据分阶段日志继续采样，但均不作为 beta 阻塞项。
  - 2026-08-27 修复日记事项重载后重复列出已选次标签，以及只删除标签后共享候选未刷新的问题；`Diary.AppTests` 285/285、应用构建和 Linux X11 `ui-smoke` 通过，该项已归档到 `CompletedWork.md`，不增加新的待办。
  - 2026-08-28 为非当天的直接新建和从模板新建增加日期确认及“今天不再提示”，抑制状态按本地自然日持久化并自动过期；定向测试 5/5 通过，该项已归档到 `CompletedWork.md`，不增加新的待办。
  - 2026-08-31 已提交事项增加显式本地修改模式，日期与 Tracker 关联保持只读，保存和标签自动化均不会改写 Tracker 数据；实现与测试已归档到 `CompletedWork.md`，不增加新的待办。
...
- [ ] 在 Windows 与 Linux 分别对优化后的 PostgreSQL + Redmine 日期切换至少复测三轮，确认 P95 稳定低于 300 ms，并继续拆分每事项编辑扩展创建、选项同步和视觉树构造成本。已消除无绑定事项逐条回查，单轮 Linux Debug 测试由每日期 43 次查询降至 5 次，P50 从 242.70 ms 降至 131.99 ms、P95 从 301.78 ms 降至 217.01 ms。
  - 2026-08-27 已针对“启动当天为空，首次进入有事项日期”将右侧编辑器改为页面级单实例，并在空日期后台绑定轻量占位 ViewModel 完成真实模板和布局预实现；日期结果同时改为内存构建后一次性替换集合。Linux `DISPLAY=:1` 下 4 个新进程冷切换中位数为 295.48 ms。后续仍需在 Windows HDD 上复测，并拆分剩余的数据库读取、真实 ViewModel/Tracker 扩展构造和左侧列表布局成本。
  - 同日事项切换专项已覆盖无 Tracker 和 Jira 本地 Tracker 两组：热切换 P95 中位数分别为 64.91/54.20 ms，带 Tracker 首次逐项 P95 为 87.27 ms；后续只需在 Windows 目标包与真实 Redmine 配置下确认同量级，无需为当前 Linux 数据立即增加新的缓存层。
...
- [ ] 按 [`RedmineTagAutomationTesting.md`](RedmineTagAutomationTesting.md) 实现独立 `ui-redmine-tag-automation`：配置和应用阶段继续补齐非强制/强制对照、冲突、无效目标、实例禁用和双实例隔离。基础规则保存、带标签模板空项更新/已有内容保护/强制应用、手动标签默认值与真实工时同步已由 `ui-redmine-full` 14/14 验证；三个项目、十四个稳定问题和 ViewModel 模板顺序测试已经就绪。
...
- 2026-08-31 修复“同步当天”成功结果因标题含“失败 0”而显示错误对话框；批量同步和周期同步已显式传递通知级别，统一通知管道不再按标题推断，未指定时固定为普通信息。实现与回归测试已归档到 `CompletedWork.md`，不增加新的待办。
