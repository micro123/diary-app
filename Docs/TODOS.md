...
- [ ] 查询失败时显示可理解的错误，不清空上一次成功结果。
- [ ] 支持从查询结果打开或定位到对应工作项。
- [ ] 设计并实现保存查询模型，允许用户保存常用查询条件。
- [ ] 保存查询只保存查询条件，不保存执行结果和敏感 Tracker 数据。
- [ ] 支持编辑、重命名和删除保存的查询。

### 7.4 脚本和统计复用

- [x] 统计页面迁移到统一查询接口，避免继续维护独立标签查询 SQL。
- [ ] 为脚本 API 提供只读 `QueryWorkItems` 能力。
- [ ] 脚本查询 API 只能读取宿主允许的工作项数据，不能修改模板。
- [ ] 脚本查询结果遵循相同的日期、标签匹配和排序语义。
- [ ] 增加查询 API 的权限、异常和敏感字段测试。
...
- [ ] 复核并版本化 `Diary.ScriptBase` 的 `IScript`、`IApplicationScript`、`IEditorScript` 和 `IScriptApi` 契约。
- [ ] 定义结构化 `ScriptDiagnostic`、`ScriptBuildResult` 和 `ScriptExecutionResult`。
- [ ] 定义 `ScriptDescriptor`，包含稳定 ID、显示名称、源码路径、引擎、用途、启用状态和能力声明。
- [ ] 定义 `ScriptScope`，支持工作项、日、周、月、季度、年和自定义日期范围。
- [ ] 定义 `ScriptTarget`，支持当前编辑器、当前/选中工作项、项目、Tracker Issue 和 Tracker 实例。
- [ ] 定义 `EditorScriptContext`，统一传递范围、粒度、目标和只读执行参数。
- [ ] 保留 `ExecuteDay`/`ExecuteRange` 兼容适配，新脚本使用上下文式执行入口。
- [ ] 定义 `IScriptManager`、`ScriptCatalog`、`ScriptBuildService` 和 `ScriptExecutor` 的职责边界。
- [ ] 实现脚本目录扫描、扩展名匹配、元数据读取和启用/禁用状态加载。
- [ ] 确保单个脚本发现或构建失败不会阻断其他脚本和核心启动。

### 9.2 C# 脚本引擎

- [ ] 使用 Roslyn 实现 `Diary.Script.CSharp.CSharpEngine.Build()`。
- [ ] 限制 C# 脚本可引用的程序集和命名空间，只暴露稳定脚本契约和受限宿主 API。
- [ ] 禁止脚本直接访问文件系统、网络、进程、数据库连接、服务容器和主程序对象。
- [ ] 支持应用脚本、编辑器脚本和上下文式脚本入口的识别与实例化。
- [ ] 将 Roslyn 编译诊断转换为统一诊断，保留文件名、行号、列号和严重级别。
- [ ] 为成功编译、语法错误、类型错误、入口类型错误和异常程序集引用增加测试。
- [ ] 评估脚本程序集加载上下文和卸载策略，避免反复重新加载造成内存泄漏。

### 9.3 执行、取消和权限

- [ ] 脚本默认在后台执行，禁止阻塞 UI 线程。
- [ ] 每次执行创建独立执行 ID、取消令牌、超时策略和日志上下文。
- [ ] 实现 `Succeeded`、`Failed`、`Cancelled`、`TimedOut` 和 `PermissionDenied` 结果状态。
- [ ] 捕获脚本异常并转换为诊断，不让异常传播到应用主循环。
- [ ] 实现用户取消和超时处理；无法强制终止的引擎必须明确记录限制。
- [ ] 定义 `ScriptCapability`，至少覆盖读写日记、读写 Tracker、剪贴板、UI、网络和文件系统。
- [ ] 默认只授予必要的只读能力，写入、网络和文件系统能力必须显式授权。
- [ ] 权限拒绝返回结构化结果，不得静默跳过危险操作。
- [ ] 对批量创建、批量修改和 Tracker 远程写入增加预览/确认机制。
- [ ] 检查脚本日志、错误详情和诊断导出，禁止泄露 API Key、Token 和密码。

### 9.4 脚本 API 和宿主能力

- [ ] 将现有 `IScriptApi` 的应用信息、日志、进度、日记查询和写入能力接入真实宿主实现。
- [ ] 评估按能力拆分 `IScriptApi`，降低单一接口增长和权限控制复杂度。
- [ ] 为日期范围提供宿主统一的周期计算，处理闰年、月份长度、周起始日和时区规则。
- [ ] 提供只读工作项查询 API，复用 `WorkItemQuery` 和统一标签筛选语义。
...
- [ ] 第一阶段不提供模板写入 API，脚本不接收模板 ID，也不修改模板扩展 payload。
- [ ] 为宿主 API 创建内存替身，方便测试脚本逻辑而不启动完整 UI 或真实服务。

### 9.5 缓存、目录和用户体验

- [ ] 约定脚本目录结构：`scripts/application`、`scripts/editor` 和 `scripts/cache`。
- [ ] 定义源码文件旁元数据或脚本包的格式，校验稳定 ID 和入口用途。
- [ ] 实现基于引擎名称、引擎版本、源码哈希、脚本契约版本和权限策略的缓存键。
- [ ] 源码、引擎、契约、权限或元数据变化时使缓存失效。
- [ ] 缓存使用临时文件写入和原子替换，损坏缓存自动删除并重新构建。
- [ ] 提供脚本列表、启用/禁用、重新加载、编译检查和错误详情 UI。
- [ ] 显示脚本最近执行状态、耗时、取消/超时状态和诊断信息。
- [ ] 提供应用脚本和编辑器脚本的命令入口，避免 ViewModel 直接调用具体引擎。
...
- [ ] 引擎按扩展名匹配，不支持的脚本不会被加载。
- [ ] 脚本目录发现、元数据错误、重复 ID 和启用/禁用状态测试。
- [ ] 编译成功、编译失败和结构化行列诊断测试。
- [ ] 应用脚本、编辑器脚本和上下文脚本分发测试。
- [ ] 时间粒度、日期范围和业务目标校验测试。
- [ ] 脚本异常、取消、超时和权限拒绝测试。
- [ ] 编译缓存命中、源码变化、引擎变化和契约变化后的缓存失效测试。
- [ ] 脚本只能处理宿主传入的模板结果，不能调用模板写入能力的测试。
- [ ] Tracker 使用 `PluginId + InstanceId` 定位正确实例的测试。
- [ ] 脚本查询 API 与数据库查询 API 返回一致结果的测试。
- [ ] 敏感配置不会出现在脚本日志、错误诊断和导出文件中的测试。
...
- [~] 定义 `TagAutomationContext` 和 `ITagAutomationCoordinator`；结构化结果待补。
...
- [ ] 设计同字段多规则冲突、无效目标和 Tracker 实例未启用时的处理。
- [x] 在 Tracker 实例设置页提供规则新增、编辑、删除、启用/禁用和优先级调整。
- [ ] 在核心标签编辑器提供 Tracker 规则扩展入口，按标签查看关联实例规则。
- [ ] 两个规则编辑入口共享编辑 ViewModel 或编辑服务，避免配置互相覆盖。
- [ ] 规则配置支持 schema 迁移、未知字段保留和敏感信息保护。
