...
- [~] 已定义稳定 ID、名称、API 版本、范围和能力描述符；更丰富的脚本包元数据待补充。
...
- [~] C# 引擎使用受限程序集引用和 Roslyn 语义策略拒绝文件、网络、进程、反射、数据库和 DI API；进程内执行仍属于受信任脚本模型。
- [~] 已拒绝对文件系统、网络、进程、数据库连接、服务容器和主程序对象的直接引用；强隔离仍需独立进程。
...
- [~] 每次执行已创建独立执行 ID、取消令牌、超时策略和独立上下文；日志上下文待接入。
...
- [~] 实现源码文件旁 JSON 元数据（支持 `enabled`、`apiVersion`、ID、名称、描述和能力校验）；脚本包格式待补充。
- [x] 实现基于引擎名称、引擎版本、源码哈希、脚本契约版本和权限策略的缓存键。
- [x] 源码、引擎、契约或权限策略变化时使缓存失效。
- [x] 缓存使用临时文件写入和原子替换，损坏缓存自动删除并重新构建。
- [~] 提供脚本列表、启用/禁用、重新加载、编译检查和错误摘要 UI；完整诊断详情页待补。
