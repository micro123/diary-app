...
- [x] 已将日记、Tracker 和系统交互能力分别整合为 `IDiaryApi`、`ITrackerApi` 和 `ISystemInteractionApi`；旧细分接口仅保留为宿主内部适配层。
...
- [x] 脚本管理页左侧列表项提供“执行”快捷按钮，复用统一脚本执行命令和现有可运行性校验。
- [x] 脚本管理页列表右侧固定为“执行”和“删除”快捷操作区；删除脚本需要二次确认。
- [x] 脚本不再使用 Enabled 状态控制可执行性；源码能够成功加载/编译即视为可用，失败时统一显示加载失败。
- [x] 移除脚本 capability 权限拒绝模型，默认授予已实现宿主 API；C# Worker 已接入工作项查询和 Tracker 只读实例目录。
...
- [x] 脚本管理页已支持编辑器脚本的指定日期、固定周期和自定义日期范围；工作项菜单支持当前已保存工作项目标。
