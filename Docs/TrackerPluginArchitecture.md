...
实例配置保存、启停或插件 UI 重新注册时，旧贡献必须先停止接受新操作。插件自有网络客户端若仍有在途请求，应使用租约、引用计数或等效机制延迟实际释放，直到最后一个请求完成；后台命令观察到旧实例生命周期结束时只能记录或返回可恢复结果，不能把 `ObjectDisposedException` 抛到 UI 调度器。当前 Redmine `RestClient` 已按该规则处理。
...
- 通用配置持久化、配置 schema 迁移和敏感字段加密/遮罩已完成。
- 插件诊断页面、迁移重试、实例启用/禁用和日志错误导出已完成。
- 禁用/卸载时保留配置和数据，删除数据必须显式确认。
- 外部 Redmine API 测试通过 `DIARY_RUN_REDMINE_EXTERNAL_TESTS=1` 显式启用，默认测试不依赖网络。
