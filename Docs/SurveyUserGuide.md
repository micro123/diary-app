...
页面通过“查询模式”显式决定使用 v1 还是 v2。选择兼容查询时，扩展查询区域不会参与请求；选择扩展查询后才可以配置扩展条件。
...
旧版节点不支持能力探测，因此不会出现在“已发现的新版节点能力”列表中。

## 12. 等待时间和状态提示

点击“发起调查”后，页面会进入约 3 秒的主要响应窗口。状态区域会显示当前收到的节点数量和节点返回的错误。
