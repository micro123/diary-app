...
Linux 原生 Debug App 在 `DISPLAY=:0`、1280×800 桌面会话中完成验证：生命周期工具可在命令退出后保持独立 App 会话，并由后续 `status/run/stop` 命令接管；`ui-core-full` 为 14/14 passed，`ui-smoke` passed。Debug 冷启动到 CDP ready 为 1,438–1,455 ms，core 截图 P50/P95 为 53.45/71.82 ms，smoke `DOM.getDocument` P50/P95 为 8.77/11.01 ms。
