...
同日 `ui-extra-fields-full` 将 `Time` 字段操作切换为环形表盘的真实指针事件：小时环验证 8 个方向数字，按下并松开后必须进入分钟；分钟环验证每 5 分钟主刻度，按住拖动时弹层保持打开并预览目标值，松开后立即应用和关闭；测试使用非主刻度 `53` 验证选中数字绘制，同时限制弹层宽度不超过 270px、表盘不超过 230px。`DateTime` 时间部分复用相同流程并继续验证与日历无重叠。本机 Windows 隔离场景 8/8 通过。
...
- 后台任务进度 Flyout 已由状态服务和状态栏实现；真实长任务下载、原生文件选择器及跨进程更新期间的动态进度仍保留集成或人工验证。
- Release 不运行 CDP；“Release 包不含 CDP”和“应用 ZIP 携带稳定路径用户手册”由构建和发布包集成校验负责。
- 脚本确认和导出后打开询问的应用内对话框层已由 CDP 覆盖；真实 XLSX/CSV/DOCX/Mustache 文件落盘、原生目录选择器和系统默认程序启动仍缺少完整 UI 端到端门禁。
...
2026-08-27 扩展真实 `ui-redmine-full` 到 14 步，并针对长标签列表增加 ComboBox/MenuItem 分页定位和异步规则列表稳定采样。规则编辑器改为两行自适应卡片后，1280×800 下标签、活动、问题和删除操作无重叠；模板更新、模板应用和事项添加标签改用动态上下文菜单，避免 Transient Flyout 不可见及弹出层名称绑定失效。最终 14/14 通过，覆盖带标签模板空项更新、已有内容保护、强制应用、手动标签默认值、保存、真实同步、防重复、删除边界、配置加密和性能，报告为 `ui-redmine-full-2026-08-27T11-10-05-194Z.json`。

## 9. 2026-08-28 对话框定向回归

标准消息、脚本确认和导出后打开询问完成紧凑自定义宿主改造后，在 Windows 150% DPI 下重新执行真实窗口截图：

- `ui-database-error` 8/8 通过，最终报告为 `ui-database-error-2026-08-28T15-32-57-034Z.json`；`database.retry` 断言警告语义、标题、正文和复制入口，并生成 `manual-standard-message-warning.png`。详情标题图标与文字垂直居中，正文默认最小高度为 120px。
- `ui-extended-full` 的 `scripts.preview-run` 通过，依次验证脚本确认框的标题、可选择消息、水平“取消 / 确认”按钮，以及导出后打开询问的横向等宽选项、说明和“推荐”状态；生成 `manual-script-confirm-dialog.png` 与 `manual-script-export-open-dialog.png`。
- 本次 `ui-extended-full` 总计 9/11，最终报告为 `ui-extended-full-2026-08-28T15-32-23-073Z.json`；`scripts.ai-context` 仍因默认状态显示事项日期范围失败，`settings.mcp-setup` 仍未识别已生成的 MCP 快照。两项为本次对话框改动之外的既有失败；脚本创建、筛选、预览运行、确认、选项、历史日志、删除和性能步骤均通过。导出后打开询问已移除重复的“选择操作”，双卡等宽并铺满内容区左右两侧。
- `Diary.AppTests` 306/306 通过，Debug 应用构建 0 警告、0 错误。真实导出文件落盘、原生目录选择器和系统默认程序启动仍属于后续完整端到端边界。
