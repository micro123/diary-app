...
兼容插件由宿主创建并加载配置，实例注册时通过 `PluginHostContext` 同时接收数据库、插件配置和通用实例配置项。Tracker 配置保存时只校验已启用实例，禁用实例可保留未完成配置；敏感配置整体加密，API 层仅记录不含响应正文和凭据的结构化摘要。`TrackerPluginLifecycleCoordinator` 统一枚举实例配置、调用插件实例注册、收集失败状态，并按已启用实例注册 UI/模板贡献；配置更新事件完成重新注册后，日记编辑器会批量读取当前日期工作项绑定、重建每个工作项的 Tracker 编辑扩展并刷新卡片可见性，同一实例未保存的选择会复制到新扩展。实例卸载默认只禁用并保留配置/数据，显式删除数据时才调用插件清理契约。插件注册前，宿主会把本次发现的 manifest 集合放入兼容性上下文，校验必选依赖的存在性和版本范围；必选依赖形成环的插件不会进入服务注册。
...
工作项上传的远程协调可以从后台线程执行，但 `WorkEditorViewModel.Upload()` 完成后统一通过 Avalonia UI Dispatcher 更新内部 `UploadResults`、锁定状态和状态绑定，避免后台线程直接修改绑定集合。编辑区只展示事项级同步状态，不展示这份临时逐 Tracker 结果；失败或结果待确认通过 Toast、通知和批量结果对话框反馈。
...
事件记录页的 `DailyWorks` 使用统一的优先级、ID 排序规则：`WorkPriorities` 升序后按持久化工作项 ID 升序。日期加载先在内存中完成编辑器构建和排序，再一次性替换绑定集合，避免逐条 `Add` 触发重复的列表卡片创建与布局；复制新增和每次工作项保存后的原集合重排仍使用 `ObservableCollection.Move()`，避免破坏当前选中项，并在移动期间抑制由选择变化触发的重复保存。日期和事项切换会比较核心字段、备注与 Tracker 绑定，只在实际修改后保存；日期列表重建期间明确抑制选择变化保存，纯浏览不应修改 SQLite 主文件或 WAL，也不应增加 PostgreSQL 业务写入计数。

日记页可能在当天无事项时只显示空状态。右侧详情区在 `DiaryEditorView` 生命周期内固定持有一个 `WorkEditorView`，事项切换只替换其 `DataContext`，并解除旧 `WorkEditorViewModel.View`、关联新 ViewModel。若页面挂载 250 ms 后仍没有选中事项，会在 UI 后台优先级创建一个不进入 `DailyWorks`、不参与保存的轻量占位 `WorkEditorViewModel`，让同一编辑器以透明、不可交互状态在真实详情区尺寸下完成绑定、模板、Tracker 子区和两轮布局；第一个真实事项接管后立即退出预热可见状态并释放占位模型，后续空日期恢复完全隐藏。这样不会在多个事项间共享 ViewModel 状态，也不会让占位数据进入数据库。左侧事项摘要仍按数据项独立生成，Tracker 子编辑器继续由各自扩展管理。
...
核心编辑器从 `TrackerUiContributionRegistry` 获取按实例创建的 `ITrackerUiContribution`，不直接依赖 Redmine 具体 ViewModel。模板只保存核心字段和默认标签，Tracker 活动、问题等默认值由标签规则推导；当前 Redmine UI 仍保留 `IRedMineUiData` 和部分 Redmine 专用数据缓存，用于管理页和选择器。导入或迁移事项的核心只读标记由宿主同步到每个 `TrackerEditorTabItem`：Tab 头仍可切换以查看不同实例的历史绑定，但扩展内容区整体禁用，因此第三方 Tracker 也不能绕过核心只读边界修改关联。
核心编辑器的一般信息区域使用统一标签列和内容列，耗时由一个输入框同时接收数字小时和时间表达式，并与快捷时长、优先级在同一行展示；输入按 `Enter` 或失焦应用，按 `Esc` 恢复最近已应用值，格式说明收纳在 Tooltip 中。日记页左侧工作项列表按标题、耗时/标签、保存与同步状态分层展示，列表项只依赖 `WorkEditorViewModel` 的通用状态属性，不引入具体 Tracker 类型。
...
- 批量同步已具备执行前预览、确认、逐条通知和仅重试已确认失败项；数据库不可用时由核心页面提供重试连接、打开设置和导出诊断日志入口，Tracker 诊断页提供实例级重试和诊断导出。远程同步队列、结果不确定项查询和批量预览的 Tracker 实例筛选仍需完善；Jira/Redmine 本地绑定保留防重复所需的上传状态，但编辑器不展示最近一次逐 Tracker 结果明细。
...
标签编辑器采用左侧标签导航和右侧详情页签，基础信息、元数据、Tracker 自动化操作、附加字段分别维护。左侧列表支持按名称即时过滤，并在每个标签下显示附加字段数和元数据数；过滤只改变可见集合，不改变标签数据。标签页内的 Redmine 自动化规则按“启用、活动、问题、删除”水平排列，并复用外层当前标签，不重复显示标签选择；Tracker 全局设置中的规则编辑器仍保留标签选择。附加字段页签只显示字段摘要；新增或编辑字段通过二级对话框完成，确认后回填主页面草稿，并立即按 `SortOrder`、`FieldKey` 升序稳定重排，主页面保存时统一提交。

日志编辑器不展开附加字段，只提供独立“附加信息”对话框，并按标签分组编辑；文本、多行文本、整数、小数、三态布尔、日期、时间、日期时间和选项字段分别使用对应控件，所有类型均保留空值语义。迁移工具导入的只读工作项不显示附加信息入口，普通事项因 Tracker 同步而锁定时也不能修改附加字段。字段定义可配置同类型默认值，只在新建事项或新增标签时预填，不回填历史数据。按钮 Tooltip 显示截断预览。附加字段编辑不触发脚本执行；C#/Lua/Python 的工作项查询和事项目标 DTO 通过 `FieldKey` 同时只读访问事项实际值与字段定义默认值，两者不会自动合并；MCP 字段定义查询也披露配置默认值。
