...
- `Time` 使用紧凑的 24 小时环形选择器：小时环固定绘制每 3 小时一个的 8 个方向数字，松开后进入分钟；分钟环每 5 分钟绘制一个数字，可按住拖动并精确到 1 分钟，松开时立即应用并关闭。非主刻度被选中时仍额外显示准确数字，保存格式保持 `HH:mm:ss`；
- `DateTime` 使用 `CalendarDatePicker` 与同一环形时间控件组合，保留已有时区偏移并提供显式清空；
- `Choice` 使用字段定义中的选项下拉框并提供显式清空。

已同步并因 Tracker 状态锁定的普通工作项，其标题、工时、优先级、备注、模板操作和 Tracker 关联继续保持只读；标签与附加字段属于本地分类数据，仍允许添加、移除、编辑、清空和保存。这些本地修改不会回写 Tracker，也不会改变已有的工时同步状态。

迁移工具导入的旧版本工作项属于完全只读的统计记录，标签也不可添加或移除。由于旧软件没有附加字段功能，迁移记录不显示附加信息入口，也不展示当前数据库中基于同名标签定义的附加字段。SQLite/PostgreSQL provider 仍会拒绝对只读工作项执行标签和附加字段写入，避免只通过 UI 绕过限制。
