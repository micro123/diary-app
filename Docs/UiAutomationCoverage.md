...
| 3.3 | 日历上下文菜单和日期/周/月/季度/年动作 | `Automated + Unit/Integration` | core 通过标准上下文菜单键验证紧凑日期同时提供日/周操作、月份标题同时提供月/季度/年度操作，并验证完整月历外层宽高均不小于内部模板、四周边框不会被裁切；命令路由和脚本目标由 ViewModel/脚本测试验证 |
