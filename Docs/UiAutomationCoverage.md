...
| 3.2 | 回到今天、新建、使用今天、草稿跨导航保存 | `Automated` | core 验证日历只翻到上个月而当天仍保持选中时，按钮可恢复当前月份；smoke 验证事项日期修正、本地保存和跨导航保留 |
...
| 3.5 | 标题、日期、工时、优先级、备注等通用字段 | `Automated` | smoke 覆盖统一耗时输入的 Enter、Esc、失焦应用，以及标题、日期、模板工时和保存；完整格式组合由单元测试承担 |
