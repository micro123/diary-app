...
| 3.2 | 固定一周、逐周浏览、回到今天、新建、使用今天、草稿跨导航保存 | `Automated` | core 验证 7 个日期、滚轮逐周浏览且不改变选中日期、跨月后恢复当前周；smoke 验证事项日期修正、本地保存和跨导航保留 |
...
| 3.3 | 日历上下文菜单和日期/周/月/季度/年动作 | `Automated + Unit/Integration` | core 使用真实鼠标右键验证非选中日期会变为当前选中并打开日/周菜单，同时验证今天与选中状态分离；月份标题通过标准上下文菜单键验证月/季度/年度操作，并验证完整月历外层宽高均不小于内部模板、四周边框不会被裁切；命令路由和脚本目标由 ViewModel/脚本测试验证 |
...
| 3.7 | 9 类附加字段编辑器、空值、格式和只读 | `Automated` | extra-fields 逐控件验证类型化编辑、三态布尔、Choice/Time 清空与重设、日期时间控件无重叠、切换事项后的值持久化、停用字段历史值及迁移只读事项；转换和数据库契约另有定向测试 |
