...
| 3.3 | 日历上下文菜单和日期/周/月/季度/年动作 | `Automated + Unit/Integration` | core 使用真实鼠标右键验证非选中日期会变为当前选中并打开日/周菜单，同时打开周度工时概要对话框；月份标题通过标准上下文菜单键验证月/季度/年度操作、月度工时概要对话框和脚本组前分隔符，并验证完整月历外层宽高均不小于内部模板、四周边框不会被裁切；概要分类与小时数守恒、空脚本子菜单、命令路由和脚本目标由 ViewModel/模型测试验证 |
...
| 3.6 | 标签创建、选择、模板应用和最近标签 | `Automated + Unit/Integration` | smoke 创建标签并验证模板新建；Redmine 真实流程验证独立“从模板更新”和“应用模板”入口、空项补全、已有内容保护、标签替换及规则触发；ViewModel 测试覆盖模板顺序、标签清空重建、已有标签保持和数据库持久化 |
| 3.7 | 9 类附加字段编辑器、空值、格式和只读 | `Automated + Unit` | extra-fields 逐控件验证类型化编辑、三态布尔、Choice 清空与重设、Time 小时八方向数字、分钟 5 分钟主刻度、非主刻度选中数字、表盘尺寸、拖动预览及分钟松开自动应用关闭；DateTime 验证日历与同一表盘组合、布局无重叠和值持久化，并覆盖停用历史值和迁移只读入口；Headless 测试验证时间显示、空值和只读状态 |
...
| 3.10 | 已同步事项锁定、本地修改模式、删除警告和取消 | `Automated + Unit` | Redmine 验证默认锁定、“修改”后仅开放本地字段、日期与 Tracker 保持禁用、保存后重新锁定、防重复同步及仅影响本地的删除警告；单元测试验证保存请求保持原日期并跳过 Tracker 扩展与标签默认值 |
| 3.11 | 标签自动化填充 Tracker Issue/活动 | `Automated` | Redmine 创建并保存规则，验证模板更新、模板应用和手动添加标签均填充 Issue/活动；独立专项套件继续补冲突、无效目标和双实例组合 |
...
| 10.1 | 工作项模板创建、字段、标签、主标签删除后重加、应用和草稿保留 | `Automated` | smoke 验证主次标签候选切换、删除主标签后清空并重新添加主标签，以及模板应用链路 |
...
| 11.3 | 通用批量同步预览、失败提示和重试 | `Unit/Integration` | 当前 UI 套件覆盖单事项真实同步、锁定和防重复；通用批量协调由 ViewModel/服务测试承担 |
