...
- UI 运行时迁移到 Avalonia 12.1.1：同步升级 Semi/Ursa/AvaloniaEdit/TreeDataGrid/SVG/Behaviors，以 Optris 替换 Projektanker、以 ScottPlot 替换会混入 Avalonia 11 的 LiveCharts，并将 Debug CDP 升级为 Avalonia 12 通用包；保持现有主窗口、统计柱状图和主要交互。自定义标题栏改用 Avalonia 12 的窗口元素角色，修复左上角应用图标、版本号、主题和设置按钮无法点击的问题；主题按钮显式切换应用级主题，不再只改变标题栏反色区域。Debug/Release 构建与测试零失败，Windows 8 个本地 UI 套件、Linux WSLg core/smoke 及 Windows/Linux 发布验证通过；Ursa 2.2 独立窗口装饰层与反色标题栏的配色差异列为已知兼容项。
