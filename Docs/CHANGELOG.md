...
日期：2026-08-31
- 修复日记页未选中事项时仍显示一个无文字“修改”按钮的问题；空选择或绑定尚未建立时明确隐藏该按钮，只有已同步事项才显示本地修改入口。
- 修复模板编辑器删除主标签后再次打开“添加标签”仍可能显示旧次标签候选的问题：标签菜单改为每次打开时按当前模板状态重新创建，删除主标签会清空次标签并立即恢复主标签候选；CDP 回归覆盖主标签和次标签添加、删除主标签及重新添加主标签的完整链路。
- 修复“同步当天”全部成功时仍弹出“操作失败”对话框的问题：批量结果不再使用包含“失败 0”的标题，并显式传递成功、警告或错误通知级别；统一通知宿主与通知历史不再根据标题猜测级别，未指定类型时固定显示普通信息。
- 已提交事项的工具栏新增“修改”按钮：默认继续锁定全部编辑内容，进入本地修改模式后可修改标题、耗时、优先级、备注、标签、附加字段并使用模板；日期始终保持原值，所有 Tracker 关联控件保持只读，保存时也不会调用 Tracker 扩展或改写远程绑定。保存成功或离开事项后自动退出修改模式。
- 重设计标准通知、脚本确认和脚本选项对话框：移除厚重外壳与重复标题，消息支持局部选择复制和超长滚动，空正文不显示详情卡；通知详情区增加稳定默认高度，脚本确认使用水平“取消 / 确认”按钮，导出后“打开文件 / 暂不打开”改为带说明和推荐状态、铺满内容区的等宽双列卡片。
- 当前日记日期不是今天时，“新建事项”和“从模板新建”会先显示目标日期确认；确认时可选择“今天不再提示”，该选择按本地自然日持久化并在次日自动失效，取消不会替换当前编辑项。
- 移除事项编辑区的“最近一次同步结果”明细卡片，不再展示临时的 Tracker 结果、远程 ID 和尝试时间；同步失败、部分失败或结果待确认仍通过 Toast、通知和批量结果对话框提示，已同步状态及重复提交保护保持不变。
- 日历日期菜单和月份标题菜单新增“周度工时概要”“月度工时概要”：只读对话框统计范围事项总数、总小时数，以及已提交、未提交、不能提交或提交失败三类的事项数和小时数；分类复用快捷同步资格口径，不保存草稿或触发远程提交。
- 重做 Redmine 标签自动规则卡片：状态与删除操作独占首行，标签、活动和问题映射在第二行按宽度伸缩，1280×800 下不再重叠；日记事项的模板更新、模板应用和添加标签菜单改为稳定的动态上下文菜单，避免弹出层不可见或命令绑定失效。
- 增加内部 Redmine 标签自动化测试夹具工具和专项测试设计：幂等维护基础、冲突、生命周期三个项目及十四个稳定问题，覆盖非强制保留、强制覆盖、规则冲突、无效目标、多实例、真实同步，以及带标签模板的“应用模板”“更新自模板”和主次标签顺序；凭据文件默认忽略，不进入仓库。
- 修复日记事项重载后“添加标签”候选可能再次列出已选次标签的问题：候选排除统一按标签 ID 判断，不再依赖不同查询结果对象的完整相等；同时修复只删除标签并保存时未刷新共享标签缓存的问题，已删除标签会立即从日记候选中消失。
- 标签编辑器新建标签时，保持默认黑色且未修改颜色会生成饱和度和亮度受控的随机色，修改颜色后保留最终选择；创建成功后自动选中新标签。移除底部取消按钮并忽略 `Esc` 等关闭请求，只有“保存更改”可以关闭。
- 模板编辑器的“添加标签”改为与日记编辑器一致的层级候选：无标签时只显示已启用主标签，已有标签时只显示未选择的已启用次标签，停用标签始终隐藏；移除底部取消按钮并忽略 `Esc` 等关闭请求，只有保存成功后关闭。
- Redmine 标签自动化规则增加“强制修改”标志：关闭时只填充空的 Activity/Issue，开启时覆盖已有值；历史规则和旧标签共享包默认关闭，新建规则默认开启，标签导入导出会保留该设置。
- 修复通知中心“清空全部”因引用不匹配的 Fluent 主题资源而显示为普通黑色的问题；改用 Semi 主题危险色，与“清空已读”形成明确区分。
- 新增日记 `Alt+J/K/L/;` 日期导航：`J` 前一天、`;` 后一天、`K` 后一周、`L` 前一周；主窗口在键盘隧道路由阶段捕获并优先按物理键识别，因此事项标题、备注等子控件获得焦点时仍可使用。快捷键仅在日记记录页面显示时生效，不提供会与平台返回手势冲突的 `Alt+方向键`。
...
- 事项编辑器在“使用今天”后增加紧邻的“从模板更新”和“应用模板”按钮：更新操作选择模板后只补空标题、零工时，且仅在当前无标签时添加模板标签；应用操作覆盖标题、工时并先清空后重建标签。已同步或其他锁定事项保持不可编辑。
