...
### 3.1 高：XLSX 工作表名称参数与 wire 契约不一致
...
但 XLSX 处理器当前只识别大小写敏感的 `sheetName`。因此用户按照公共命名约定使用 `sheet_name` 时，会得到格式选项无效错误。格式描述符也没有返回选项 schema，用户无法通过 `list_formats` 发现正确键名。
...
1. 统一 wire 层正式键名为 `sheet_name`。
2. 过渡期间兼容 `sheetName`，避免破坏已有脚本。
3. 在格式 descriptor 中增加选项 schema，至少说明名称、类型、默认值和约束。
4. 为 C#、Lua、Python 各增加一个设置工作表名称的可运行示例。

### 3.2 高：无效单元格值被误报为可重试的宿主故障
...
### 3.5 中高：模板表格绑定与“循环模板行”之间的语义没有说明清楚
...
### 3.6 中：部分公开字段和 capability 会被静默忽略
...
### 3.8 中：目录令牌和文件 ID 的生命周期没有写入用户文档
...
| 合并单元格 | 是 | 否 | 是 | 是 |
| 合计 | 是 | 是 | 是 | 是 |
| 模板标量绑定 | 按 descriptor | 是 | 是 | 是 |
| 模板多行表格绑定 | 需明确语义 | 不适用/需明确 | 需明确 | 需按格式说明 |
| 用户可配置样式 | 需明确 | 否 | 需明确 | 需按字段说明 |
| 格式选项 schema | 待补充 | 待补充 | 待补充 | 是 |

表格中的“否/待确认”不是最终 API 决策，修复时应以实际实现和测试结果为准；重点是不要让用户通过猜测能力来构造请求。
...
- `sheet_name` 与 `sheetName` 的兼容/统一行为有测试覆盖；
