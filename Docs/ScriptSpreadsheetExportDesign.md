...
脚本管理页面只展示最近一次脚本目录加载产生的目录诊断；启动加载继续写应用日志，不再额外保存一份与首次目录诊断重复的 UI 快照。API Reference 内容按页签延迟加载，首次进入页面不得为了不可见的文档内容同步读取和解析完整 Markdown。脚本、诊断、历史和运行日志集合刷新应尽量合并为一次 Reset 通知，避免逐项触发布局和绑定更新。
