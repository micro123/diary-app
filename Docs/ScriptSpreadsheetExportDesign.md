...
“打开文件”和“暂不打开”是两个明确选项，以等宽卡片水平排列；默认的“打开文件”显示“推荐”，每项显示后续行为说明，更多选项按宽度自动换行。禁止右上角关闭。选择“暂不打开”返回 `UserDeclined`，而不是 `Cancelled`；只有脚本取消、Worker 终止或应用退出才走外部取消路径。
...
脚本管理页面只展示最近一次脚本目录加载产生的目录诊断；启动加载继续写应用日志，不再额外保存一份与首次目录诊断重复的 UI 快照。管理页不内嵌或解析 API Reference，完整语言文档只由脚本编辑器按需交给系统打开。脚本、诊断、历史和运行日志集合刷新应尽量合并为一次 Reset 通知，避免逐项触发布局和绑定更新。
...
- `ConfirmAsync` 使用脚本专用自定义确认框，不复用应用内通用 MessageBox；标题和消息分区显示，消息支持选择复制和局部滚动，“取消 / 确认”按钮水平排列，关闭请求等价于取消。
- 选项对话框的消息区域支持选择复制和局部滚动；导出后打开询问的两个操作使用等宽横向卡片并显示说明和推荐状态。
...
- `ConfirmAsync` 的确认、取消和窗口关闭分别返回 `true`、`false`、`false`，长消息保持可选择和可滚动；
...
4. [x] 增加数据模板管理页面，支持导入、重新校验、启用/禁用和归档；
