...
本文定义 DiaryApp 客户端应用包更新的目标架构、清单协议和安全应用流程。当前已实现 `Diary.Update` 共享事务协议和裁剪后的 `Diary.Updater` 自包含单文件 CLI，更新协议 JSON 使用源生成元数据，覆盖计划校验、更新器身份校验、逐文件 Blob 或完整包下载、文件替换、事务日志、失败回滚、异常恢复、更新器交接和应用重启。`UpdateServer/` 已提供仅依赖 Python 标准库的局域网同步服务，从 GitHub Release 校验并同步三个发布维度，生成完整包快照、逐文件内容缓存和规范化清单。客户端在已安装清单可信且满足 `minIncrementalSequence` 时比较实际安装文件，仅下载新增或变化内容；清单缺失、损坏或跨度不支持时回退完整包。更新后业务初始化失败时可由用户确认回滚；文件替换因外部进程占用失败时，更新器会保留具体错误、恢复旧文件并尝试重启上一版本。跨事务内容缓存、并发下载、断点续传和真实上一版本端到端门禁仍待实现。
...
- 未压缩字节数；
- 原始文件 SHA-256；
- Linux 可执行属性；Windows 入口由 `.exe` 路径和已知入口规则识别，不复用 Unix executable 标记；
...
完整包与逐文件内容下载使用独立接口，避免清单查询与文件传输共用短超时策略：
...
当前 HTTP 实现已经消费服务端完整包和内容 Blob 接口，两者都在临时文件中流式写入并校验长度与 SHA-256 后提交。批量内容包、本地持久缓存和并发调度仍按同一文件身份规则作为后续扩展。
...
当前主程序已完成步骤 1 至 6：设置页可配置服务器地址、`stable`/`preview`/`local` 频道和 `Auto`/`standard`/`python313` flavor；`Auto` 在 Windows 安装目录存在 `python/` 时选择 `python313`，Linux 固定选择 `standard`。`local` 包由本机工具注入独立、单调递增且与包内 `AppSequence` 一致的构建序号及 `BuildChannel=local`，再通过受认证接口直接发布到局域网服务器，不经过 GitHub Tag/Release。由于时间型 local sequence 大于正常提交计数，客户端在用户主动从 local 切回 `stable`/`preview` 时按目标频道重新比较，避免测试包把安装永久锁定在 local。启动 15 秒后可以后台检查，只有发现新版本才主动提示；手动检查会反馈已是最新、没有精确发布维度、服务暂时不可用、清单无效或更新器协议过低。满足增量条件时先生成实际文件比较结果并逐个下载变化 Blob，否则下载完整包并安全解压；两条路径生成相同事务计划。
...
- 所有内容先下载到用户数据目录的事务目录，不直接写安装目录；
- 完整包和内容 Blob 都下载到事务专属临时文件，完成长度和 SHA-256 校验后再提交到事务暂存目录；
- 支持取消和断点续传属于后续增强，第一阶段可以重新下载单个失败文件；
- 每个文件下载完成后立即验证长度和 SHA-256；
- 同一哈希文件跨事务持久化复用仍属于后续缓存增强；当前逐 Blob 路径只在本次事务中保存变化文件；
- 暂存完成后再次验证整个更新计划；
...
- Windows 准备阶段若 `Diary.Mcp.exe` 属于本次替换或删除集合，会额外探测独占写入能力；外部 AI 客户端仍持有 MCP 进程时必须在主程序退出前明确提示用户结束 MCP 会话。若准备后发生占用竞态，更新器回滚时对已经等于备份内容的目标跳过重复覆盖，并尝试重启上一版本，由主程序显示事务中的具体占用错误。

## 10. 应用更新事务

### 10.1 外部更新进程

更新替换必须由独立进程执行。`Diary.Updater` 是不引用 UI 框架的最小化 CLI，并按目标 RID 发布为裁剪后的自包含单文件工具。主程序在应用更新前将更新器复制到用户更新目录，从该目录启动，避免更新器依赖正在被替换的安装文件。
...
2. 完整包与逐 Blob 路径都会将已经通过探针和哈希验证的目标版本更新器复制到 `<LocalApplicationData>/Diary.App/updates/bootstrap/<transactionId>/`，复制结果再次按哈希验证，然后从该副本启动。这样旧主程序也能直接使用目标协议完成更新；引导副本是事务基础设施，不属于安装清单、完整包受管理文件集合或删除列表。
...
```text
Diary.Updater --apply <transaction.json> --wait-pid <pid>
Diary.Updater --recover <transaction.json> --wait-pid <pid>
```
...
如果新程序已成功启动但随后业务初始化失败，不自动降级数据库；应用包回滚与数据库降级是不同风险域。当前主程序会明确提示风险，并仅在用户确认后启动事务专属引导副本回滚程序文件、验证旧入口哈希并重启旧版本。数据库迁移仍依赖现有备份和兼容性检查处理。
...
主应用发布结束后会将目标 RID 的 `Diary.Mcp` 多文件 apphost 安全合并到主发布目录。传给合并脚本的源目录和目标目录使用等价的目录点段形式（Windows 为 `目录\.`，Unix 为 `目录/.`），避免 Windows 命令行中尾随反斜杠转义结束引号而吞并后续参数；合并仍需校验同名文件大小和 SHA-256，并拒绝大小写冲突。
...
Windows 本地工具默认同样要求 Quarto 并重新渲染用户手册；仅在验证升级传输和事务链路、且 `_output` 已存在有效 HTML/PDF 时，可显式指定 `-ReuseExistingManual` 复用产物。该模式必须警告手册可能过期，不得用于正式发布。
...
### 17.4 Windows standard 本地真实升级记录

2026-08-25 使用本地更新服务验证 `1.0.0-r20260821122410` 到 `1.0.1-r20260825144922`：

- 逐文件增量路径请求 103 个内容 Blob、0 个完整包，下载 27,135,288 bytes；对应完整包为 91,455,359 bytes，减少 64,320,071 bytes（70.33%）；
- 增量计划包含新增 62 个、替换 41 个、删除 0 个文件；独立 `Diary.Updater.exe` 返回 0，事务状态为 `Applied`，目标 445 个文件全部通过长度和 SHA-256 校验；
- 删除可信安装清单后正确回退完整包：0 个 Blob、1 个完整包请求，下载 91,455,359 bytes，更新后 445 个文件全部校验通过；
- 准备阶段占用 `Diary.Mcp.exe` 时，在任何 Blob 或完整包请求前返回可操作提示；准备完成后才出现占用时，Updater 返回失败并将事务恢复为 `RolledBack`，旧应用入口和 MCP 文件内容均保持原值。

该记录覆盖 Windows standard 的本地增量、完整包兜底和 MCP 占用回滚。Windows Python flavor、Linux、初始化失败以及进程被强制终止后的恢复仍属于发布门禁剩余项。
...
- [~] Windows standard 本地真实增量、完整包兜底和 MCP 占用回滚已通过；仍需完成 Windows Python flavor、Linux、初始化失败和进程中止恢复门禁。

### 阶段二：逐文件增量

- [x] 实现实际文件哈希比较和更新计划；
- [x] 为客户端实现内容 Blob 下载适配器；
- [x] 只下载变化内容，保留完整包兜底；
- [~] 已提供累计字节进度，仍需增加跨事务缓存、并发限制和可视化进度 UI。
...
源包校验、逐文件/完整包暂存、外部更新进程、更新器自举、事务替换、稳定性确认和回滚闭环已经落地；下一阶段重点是跨事务缓存、并发与断点续传，以及真实跨版本发布门禁。若未来脱离受控局域网，必须重新评估端到端信任和清单签名。
