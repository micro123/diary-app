...
本文定义 DiaryApp 客户端应用包更新的目标架构、清单协议和安全应用流程。当前已实现 `Diary.Update` 共享事务协议和裁剪后的 `Diary.Updater` 自包含单文件 CLI，更新协议 JSON 使用源生成元数据，覆盖计划校验、更新器身份校验、文件替换、事务日志、失败回滚、异常恢复、更新器交接和应用重启基础能力；`UpdateServer/` 已提供仅依赖 Python 标准库的局域网同步服务，从 GitHub Release 校验并同步三个发布维度，生成完整包快照、逐文件内容缓存和规范化清单。主程序已经形成完整包更新闭环：检查更新、用户确认、流式下载、完整包与逐文件校验、安全解压、计划生成、引导更新器启动、正常退出、事务替换、新版本重启、稳定性确认和清理；更新后业务初始化失败时可由用户确认回滚程序文件并重启旧版本。逐文件 Blob 增量下载、断点续传和真实上一版本端到端门禁仍待实现。
...
完整包下载使用独立接口，避免清单查询与大文件传输共用短超时策略：
...
当前 HTTP 实现已经消费服务端完整包接口。逐文件 Blob、批量内容包和本地目录测试源将在阶段二按同一文件身份规则扩展。
...
- 当前完整包下载到事务专属临时文件，完成长度和 SHA-256 校验后再原子改名为 `package.zip`；
...
2. 当前完整包实现将已经通过探针和哈希验证的目标版本更新器复制到 `<LocalApplicationData>/Diary.App/updates/bootstrap/<transactionId>/`，复制结果再次按哈希验证，然后从该副本启动。这样旧主程序也能直接使用目标协议完成更新；引导副本是事务基础设施，不属于安装清单、完整包受管理文件集合或删除列表。
...
第一阶段的源包校验、暂存、外部更新进程、更新器自举、事务替换、稳定性确认和回滚闭环已经落地；下一阶段增加逐文件下载和真实跨版本发布门禁。若未来脱离受控局域网，必须重新评估端到端信任和清单签名。
