...
- 周/月快捷同步从日历上下文菜单进入，先自动保存当前事项，再按范围批量读取核心事项和 Tracker 绑定；只提交通过 `ValidateUpload()` 完整性检查的事项，并在首个 Tracker 或事项失败时终止。
...
- 上传前确保核心工作项已保存。
- 普通单项和当日预览同步中，一个实例失败不短路其他实例。
- 已锁定或未配置实例返回 `Skipped`。
- 上传不回滚本地保存。
- 成功或失败状态按 key 保留在协调结果中，用于事项状态判断和失败提示，不在编辑区展示逐 Tracker 明细。
- 重试只执行失败或可重试的实例。

### 9.3 周/月快捷同步

周/月快捷同步用于无需逐项选择的受控批量提交，和当日批量预览使用不同的失败策略：

```text
自动保存当前事项
  -> 按周或自然月查询核心事项
  -> 批量预取备注、标签、附加字段和各 Tracker 绑定
  -> 按日期、事项 ID 顺序检查可同步性
  -> 跳过导入、已同步、同步中/结果待确认、0 工时和 Tracker 信息不完整事项
  -> 对首个可同步事项按 Tracker 顺序提交
  -> 任一 Tracker 失败或结果不确定时立即终止整个批次
  -> 刷新当前日并展示成功、跳过、失败、未处理及失败位置
```

扩展通过 `ITrackerEditorExtension.ValidateUpload(WorkItem)` 声明快捷批量同步所需的本地完整性。默认实现拒绝参与，避免未知插件在没有必填值校验时产生远程副作用；Redmine 校验工时、问题、活动和失效值，Jira 校验工时、Issue 绑定和本地 Issue 有效性。多 Tracker 事项只要任一尚未锁定的目标扩展校验失败，就整条跳过，不提交其中任何 Tracker。

快捷同步使用 `UploadUntilFailureAsync()`：已锁定实例继续作为已同步跳过；第一个远程失败或结果不确定后不再调用同一事项的后续扩展，也不再处理范围中的后续事项。此前已经成功的远程提交不回滚，明细必须准确报告停止位置和剩余未处理数量。

日期菜单和月份标题菜单还分别提供“周度工时概要”和“月度工时概要”。概要是只读统计，不自动保存当前草稿，也不调用远程 API；它复用周/月范围读取及 `PeriodTrackerUploadPolicy` 的提交资格口径，将范围内事项互斥归类为：所有 Tracker 均已同步的“已提交”、当前具备提交条件的“未提交”，以及迁移只读、未配置/信息不完整、工时无效、同步失败/部分失败、同步中或结果待确认的“不能提交或提交失败”。对话框同时显示每类事项数和小时数，三类之和必须与范围总计一致。
...
- 上传结果可以按实例独立判断并支持失败重试；编辑区只展示事项级状态，失败明细通过提示反馈。
