...
## 2026-08-31 批量同步通知级别修复

- [x] 修复“同步当天”全部成功时，标题中的“失败 0”被通知宿主误判为错误并显示“操作失败”的问题；成功结果标题不再包含失败计数，失败和结果待确认分别使用错误与警告语义。
- [x] `NotifyEvent` 增加显式消息级别参数，主窗口标准消息、状态摘要和通知历史直接使用该级别；未指定时固定为普通信息，不再按标题文字推断。

## 2026-08-31 已提交事项本地修改模式

- [x] 已提交普通事项在顶部工具栏显示“修改”按钮；进入后开放标题、耗时、优先级、备注、标签、附加字段和模板操作，并使用“本地修改中”状态提示。保存成功或离开事项后恢复锁定。
- [x] 日期在界面和保存请求中保持原值；Tracker 编辑区在本地修改期间整体只读，保存请求跳过 Tracker 扩展，标签自动化不向任何 Tracker 实例应用默认值。迁移导入事项继续完全只读且不显示修改入口。

## 2026-08-28 非当天新建事项确认

- [x] 直接新建和从模板新建在目标日期不是本地今天时统一显示日期确认，取消后不替换当前编辑项；确认时可勾选“今天不再提示”，按本地日期持久化并在次日自动恢复。
- [x] 补充当天跳过、非当天提示、当日抑制、过期抑制及对话框确认/取消测试，定向测试 5/5 通过。

## 2026-08-28 精简工时同步结果展示

- [x] 移除事项编辑区的“最近一次同步结果”卡片，不新增同步结果持久化或周/月结果回填；单项同步失败与结果待确认继续使用 Toast，当天批量和周/月快捷同步继续使用通知或结果对话框报告失败位置与原因。
- [x] Redmine UI 自动化改为验证已同步状态、编辑锁定、同步按钮禁用和快捷键防重复，不再依赖远程 ID、尝试时间等编辑区明细。

## 2026-08-27 周度与月度工时概要

- [x] 日期右键菜单新增“周度工时概要”，月份标题右键菜单新增“月度工时概要”；两者复用周一至周日和自然月范围，并通过统一只读对话框展示。
- [x] 概要统计事项总数、总小时数，以及已提交、未提交、不能提交或提交失败三类事项数和小时数。所有 Tracker 均已同步才算已提交；具备提交资格算未提交；迁移只读、配置/数据无效、失败、部分失败和结果待确认均归入异常类别，三类数量与小时数严格汇总到总计。
- [x] 补充分类守恒、空范围、对话框格式和日历菜单范围测试；Debug 解决方案构建 0 警告、0 错误，App 测试 290/290 通过。全新默认 UI profile 已真实打开并关闭周度、月度概要对话框，套件随后停在更后面的既有完整月历重开断言，与本次功能无关。

## 2026-08-27 标签新建默认色与保存关闭

- [x] 标签编辑器将默认黑色作为“尚未选择颜色”的初始状态；用户未修改颜色时，新建标签使用饱和度和亮度受控的随机色，颜色值发生修改后保留最终选择。创建成功后按标签 ID 重新加载并选中新标签；当前过滤条件无法显示新标签时自动清空过滤。
- [x] 移除底部取消按钮并忽略 `Esc` 等宿主关闭请求，只有“保存更改”能够关闭标签编辑器；标签导出选择等内部对话框仍可单独取消。App 测试 283/283 通过，`ui-smoke` 验证随机默认色、新建后选中、取消按钮移除、`Esc` 不关闭和保存关闭，并直接断言两个标签的持久化颜色均非黑色，报告为 `ui-smoke-2026-08-27T08-21-27-358Z.json`。

## 2026-08-27 模板标签候选与保存关闭

- [x] 模板编辑器按当前模板标签状态动态生成候选：无标签时只列启用主标签，已有标签时只列未选择的启用次标签，停用标签始终排除；添加和移除后即时刷新候选及按钮状态。移除底部取消按钮并忽略 `Esc` 等宿主关闭请求，只有保存成功后关闭。
- [x] App 测试 282/282 通过，并覆盖保存失败时保留对话框及恢复内存模板；`ui-smoke` 验证候选标签添加、取消按钮移除、`Esc` 不关闭、保存后关闭及模板持久化，报告为 `ui-smoke-2026-08-27T08-01-02-662Z.json`。用户手册同步更新行为说明和模板编辑关键区域截图。

## 2026-08-27 标签自动化覆盖策略

- [x] Redmine 每条标签自动化规则增加“强制修改”标志；关闭时保持 `OnlyIfUnset`，开启时按规则目标替换已有 Activity 和 Issue。历史配置通过 schema 2 → 3 迁移显式补为关闭，旧标签共享包缺失字段时同样按关闭处理；界面新建规则默认开启，导入导出保留该行为。Redmine 单元测试 30/30 通过（另有 9 项外部 API 测试按环境开关跳过），真实 Redmine UI 12/12 通过，报告为 `ui-redmine-full-2026-08-27T07-26-05-301Z.json`。
...
- [x] 在日期行“使用今天”后增加紧邻的“从模板更新”和“应用模板”按钮，并复用当前模板列表；两个入口分别执行更新和覆盖应用语义。
...
- [x] 标签管理页的 Redmine 自动化规则采用水平排列；当前包含启用、强制修改、活动、问题和删除操作。标签作用域由左侧当前标签确定并隐藏重复的标签选择，同时保留 Tracker 全局设置中跨标签编辑规则的标签下拉框。
...
- [x] 历史规则默认使用 `OnlyIfUnset`，用户后续手动修改字段不被覆盖；现已允许每条规则显式开启强制修改，新建规则默认开启。
...
## 阶段 10.3：标签编辑与候选列表一致性

完成日期：2026-08-27。

- [x] 标签编辑器新建标签在未主动选择颜色时生成可辨识随机色，创建成功后自动选中；编辑器只允许保存成功后关闭。
- [x] 日记事项的添加标签候选按标签 ID 排除已选标签，避免数据库重载产生不同对象实例后重复列出已选次标签。
- [x] 仅删除标签并保存时也发布共享标签刷新，使日记和模板候选立即移除已删除标签。
- [x] `Diary.AppTests` 285/285、应用构建 0 警告、Linux X11 `ui-smoke` 通过；UI 报告为 `ui-smoke-2026-08-27T09-00-34-290Z.json`。

## 阶段 10.4：Redmine 标签自动化测试夹具与设计

完成日期：2026-08-27。测试设计见 [`RedmineTagAutomationTesting.md`](RedmineTagAutomationTesting.md)。

- [x] 增加幂等夹具工具，从本地忽略的两行凭据文件读取 Redmine 地址和 API Key，并按名称解析 Tracker/优先级，避免依赖服务器固定 ID。
- [x] 在测试服务创建基础、冲突、生命周期三个独立项目及十四个稳定问题；首次创建 3/14，第二次运行创建 0/0，确认可重复执行。
- [x] 测试矩阵覆盖默认填充、非强制保留、强制覆盖、规则顺序、无效目标、禁用、多标签、多实例、保存重载、删除标签和真实工时同步。
- [x] 带标签模板单独覆盖“应用模板”和“更新自模板”、主标签与次标签顺序、已有内容保护、失效标签边界及保存重载。
- [x] 增加两条 ViewModel 回归测试固定模板规则来源和顺序；`Diary.AppTests` 287/287 通过，`Diary.RedMineTests` 30/30 通过，外部 API 用例 9 条按默认策略跳过。
- [x] Redmine 规则卡片改为两行自适应布局，状态/删除与三个映射字段不再争用同一行；模板更新、模板应用和事项添加标签统一使用动态上下文菜单，消除 Flyout 不可见及弹出层名称绑定失效。
- [x] 真实 `ui-redmine-full` 扩展为 14 步并全部通过，覆盖规则创建保存、带标签模板空项更新、已有内容保护、强制应用、手动标签默认值、真实工时同步、防重复、删除边界、加密与性能；报告为 `ui-redmine-full-2026-08-27T11-10-05-194Z.json`。

## 阶段 10.5：通知与脚本交互对话框重设计

完成日期：2026-08-28。设计见 [`UiVisualStyleDesign.md`](UiVisualStyleDesign.md) 和 [`ScriptSpreadsheetExportDesign.md`](ScriptSpreadsheetExportDesign.md)。

- [x] 标准通知切换到紧凑自定义宿主，按消息类型显示语义图标与状态色；正文支持局部选择复制、整段复制和长内容滚动，空正文隐藏详情卡，泛化标题不再与状态重复。
- [x] 脚本 `ConfirmAsync` 使用专用确认框，消息可选择和滚动，底部“取消 / 确认”水平排列，确认、取消和关闭返回值保持原协议。
- [x] 脚本选项与导出后打开询问移除重复标题和“选择操作”，使用铺满内容区的双列等宽卡片；“打开文件”显示推荐标记，两项均提供行为说明。
- [x] `Diary.AppTests` 306/306、Debug 构建 0 警告 0 错误；`ui-database-error` 8/8 通过，`ui-extended-full` 对话框目标步骤通过并生成三张最终截图。扩展套件仍有 AI 上下文默认状态和 MCP 快照识别两个既有失败。
