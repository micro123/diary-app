...
- [x] 日记事项编辑器按可用 Tracker 编辑扩展控制“Tracker 关联”卡片可见性，无启用实例时不再保留空标题卡；ViewModel 单元测试覆盖空注册表和启用实例两种状态，Linux X11 `ui-core-full` 14/14 通过，默认无 Tracker 场景确认卡片隐藏且备注编辑框扩展至 315px。
