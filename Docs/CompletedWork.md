...
- [x] 新增 `Alt+←/→` 前后一天和 `Alt+↑/↓` 前后七天的日期导航，仅在日记记录页面当前显示时响应；复用紧凑日历原有日期切换与自动保存保护，查询、统计和 Tracker 页面不会在后台修改日记日期。核心 CDP 同时覆盖四个方向和非日记页面作用域，Linux X11 实测 16/16 步通过，报告为 `ui-core-full-2026-08-27T03-49-13-462Z.json`。
