...
幂等键、预览能力和副作用摘要已提供；宿主现在使用共享持久化幂等存储，成功追加结果会按 API 类型和幂等键保存，并在应用重启后继续识别重复请求。持久化记录只保存已提交的结果，不自动重试带副作用的请求。
...
脚本管理页面允许手动执行脚本，但脚本失败后用户可能重试。当前创建接口已接受幂等键，重复提交同一业务动作会返回重复结果而不再次追加；预览请求只返回投影记录和副作用摘要。宿主共享 `ScriptIdempotencyStore`，持久化 `logItems.create` 和 `templateLogItems.create` 的已提交结果，并按 API 类型隔离同名幂等键。
...
模板、Tracker 实例和宿主能力已经提供只读列表 API；仍可在 UI 中增加复制稳定 ID 的入口，但不影响脚本运行时契约。
...
- 已增加幂等键、预览、副作用摘要、统一进度报告和持久化幂等。
- 当前不实施历史删除、直接改写或修正/冲正 API；如业务以后确实需要更正，再单独设计追加式关联模型。
- 增加稳定游标分页。
