...
模板导出先调用 `context.exports.list_templates("xlsx")`，再按 descriptor 的精确 `template_id`、`template_version` 和 `bindings` 构造 `template={"values": ..., "tables": ..., "documents": ...}`。模板正文使用 `{{变量名}}`、`{{items.字段}}`、`{{items.字段|column}}` 和 `{{items|matrix}}` 简单标记；行循环复制模板行，列循环复制模板列，矩阵标记展开完整的 `Rows × Columns` 区域，脚本负责准备数据和其他逻辑。模板名称由文件名推断，版本为 `1.0.0`。
