...
Redmine 实例规则存储、可配置覆盖策略和 Redmine 实例设置页规则编辑器。
...
- 历史规则和旧标签共享包缺少标志时按关闭处理，避免升级后意外覆盖已有值；新建规则默认开启，用户可按需关闭。
...
- 脚本自动化的 `TagAdded` 触发复用同一新增标签入口，但与 Tracker 默认值协调器分离：持久化工作项在协调器处理后触发，草稿标签在首次保存成功后按添加顺序补发；加载、重复标签、删除标签和删除工作项不触发脚本事件。
...
核心编辑器应把手动添加、模板添加、批量添加和重复事项添加统一为标签添加事件。
...
```text
Activity 已经是“开发”
添加“加班”，规则目标为“加班”且开启强制修改
-> Activity = 加班
```

历史配置迁移和缺少该字段的旧标签共享包按关闭处理；通过界面新建的规则默认开启，避免用户新建一条明确规则后还需要额外打开覆盖行为。
...
3. 按规则在配置列表中的顺序处理。
4. 同一个字段只接受第一条有效规则。
5. 第一条有效规则关闭强制修改且当前字段已有值时不覆盖；开启时采用该规则目标。
...
启用状态、强制修改标志，并支持新增和删除规则。新建规则默认开启强制修改；已删除标签或不存在的远程目标会保留原始 ID并显示失效项；没有可用工作标签时，新增按钮会禁用并提示先创建标签。
...
规则项采用两行紧凑卡片布局：首行只放“启用”“强制修改”和删除操作，第二行按标签、活动、问题三个字段排列；标签和活动使用基础弹性宽度，问题字段获得更大的剩余宽度。各下拉框随容器伸缩，删除按钮不与字段共用列，保证 1280×800 的 Tracker 设置对话框内不会出现文字、下拉框或按钮重叠。
...
标签包只以 Tracker 插件类型和显示名称描述来源实例，不导出 InstanceId、地址或凭据。导入时由用户将来源映射到同类型本地实例，插件贡献负责校验规则字段和值是否存在；无效、无法验证或未关联的规则不写入。完整流程见 [`TagImportExportDesign.md`](TagImportExportDesign.md)。
...
真实 Redmine 夹具、CDP 场景矩阵、隔离和门禁策略见 [`RedmineTagAutomationTesting.md`](RedmineTagAutomationTesting.md)。本节继续维护核心行为和纯规则覆盖边界。
...
- 应用模板添加标签触发规则。
- 批量添加按标签添加顺序触发规则。
- 重复当前事项按复制标签添加顺序重新触发规则。
- 已存在标签不会重复触发规则。
- 加载已有工作项不会触发规则。
- 删除标签不会反向清除字段。
- 关闭强制修改时，当前字段已有值不覆盖。
- 开启强制修改时，已有 Activity 和 Issue 会被规则目标替换。
- 历史规则及旧共享包缺少标志时保持不覆盖，新建规则默认开启强制修改。
...
当前纯规则测试已覆盖配置顺序、`OnlyIfUnset`、强制修改、禁用规则、其他标签和无效目标。
...
4. [已完成] 实现 Redmine 标签规则匹配、`OnlyIfUnset` 和按规则强制修改。
