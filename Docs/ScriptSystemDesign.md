...
脚本管理页支持通过单个 `.diaryscripts` 包批量导入和导出源码及运行配置。共享包采用版本化 manifest、包内安全路径和 SHA-256 完整性校验；导入先预览，冲突默认跳过并要求用户显式授权覆盖。宿主不整包解压，限制脚本数和解压大小，并在批量写入失败时恢复备份。导入动作不立即执行脚本，但自动化脚本在重新加载后会按其运行配置参与后续调度。完整格式和安全边界见 [ScriptSharingDesign.md](ScriptSharingDesign.md)。
