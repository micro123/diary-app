...
脚本作者可从内置编辑器的 `API Reference` 入口打开随应用发布的中文语言文档。文档按 C#、Lua、Python 分开维护，并以各 Worker 当前实际暴露的上下文、宿主调用和沙箱限制为准。新建脚本流程提供按语言生成的“空白脚本”和“查询工作项”样板，样板所需能力会同步写入 metadata。
