...
后台主动回收、stdout 脚本输出和内存限制及多语言 worker 仍在后续阶段。现有脚本 V1 类型和执行结果见
...
- [~] 已实现心跳、超时、进程退出事件监听、执行通道终止诊断、惰性空闲回收和指数退避启动；退出码诊断、后台回收和更完整的重启策略待完成。
- 增加执行历史和 worker 诊断关联。
...
- [~] 已增加执行消息、执行结果、stderr 输出、调用数和请求数限制；stdout 脚本输出、内存及后台回收限制待完成。
