...
异步的，并使用独立取消令牌。当前 supervisor 已覆盖等待 HostCall 时的取消回收：返回 `Cancelled`，并将无法继续通信的 worker 标记为失败后释放。C#、Lua worker 的脚本级取消令牌传播仍是后续工作，Python worker 通过执行线程的 trace 检查取消状态。
