...
异步的，并使用独立取消令牌。当前 Worker 主循环可以在脚本执行或等待 HostCall 时接收 `cancel`，并通过 HostResult 路由解除等待；supervisor 已覆盖等待 HostCall 时的取消回收，返回 `Cancelled`，并将无法继续通信的 worker 标记为失败后释放。C#、Lua 当前没有脚本级轮询取消 API，Python worker 通过执行线程的 trace 检查取消状态。
